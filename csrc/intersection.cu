...
__global__ void ray_aabb_intersect_kernel(
    ...
    const int v = blockIdx.y * blockDim.y + threadIdx.y;//多个center时
    ...
    const float2 t1t2 = _ray_aabb_intersect(ray_o, inv_d, center, half_size);//返回一个volume的交
...
std::vector<torch::Tensor> ray_aabb_intersect_cu(
    ...
    auto hits_order = std::get<1>(torch::sort(hits_t.index({"...", 0})));//击中多个volume，取最近的那个volume的dim1
