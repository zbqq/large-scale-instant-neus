...
inline __host__ __device__ float calc_dt(float t, float exp_step_factor, int max_samples, int grid_size, float scale){
    return clamp(t*exp_step_factor, SQRT3/max_samples, SQRT3*2*scale/grid_size);//1/256 100 128
}
...
inline __device__ int mip_from_pos(const float x, const float y, const float z, const int cascades) {
    ...
    return min(cascades-1, max(0, exponent+1));//ceil(log(2,x)) + 1
}
...
inline __device__ int mip_from_dt(float dt, int grid_size, int cascades) {
    ...
    return min(cascades-1, max(0, exponent));//exponent
}
...
__global__ void morton3D_kernel(
    ...
    indices[n] = __morton3D(coords[n][0], coords[n][1], coords[n][2]);//每个点存了该点对应的morton code
}
...
// packbits utils
template <typename scalar_t>//将单个grid的8个角点映射到0,1
__global__ void packbits_kernel(
    const scalar_t* __restrict__ density_grid,//哈希表，n正方体
    const int N,
    ...
    for (uint8_t i = 0; i < 8; i++) {
        bits |= (density_grid[8*n+i]>density_threshold) ? ((uint8_t)1<<i) : 0;//若第i个点的density大于阈值，则对应8为bits的位为1
    }

    density_bitfield[n] = bits;
}

void packbits_cu(//host
...
__global__ void raymarching_train_kernel(
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> rays_o,//rays,3
    ...
    const int cascades,//max(1+int(np.ceil(np.log2(2*scale))), 1),若scale=100,则cascades=9
    const int grid_size,//128
    const float scale,//0.5,尺度因子
    const float exp_step_factor,//1/256
    ...
    while (0<=t && t<t2 && N_samples<max_samples){
        ...
        const float dt = calc_dt(t, exp_step_factor, max_samples, grid_size, scale);//从t1开始采样
        const int mip = max(mip_from_pos(x, y, z, cascades),
                            mip_from_dt(dt, grid_size, cascades));//dt*128 = [0.5,1] * 2^exponent
        // 这里的mip就相当于xpos的2对数
        const float mip_bound = fminf(scalbnf(1.0f, mip-1), scale);//1 * 2^(mip-1)
        ...
        const bool occ = density_bitfield[idx/8] & (1<<(idx%8));//离pos最近的网格点density_bit是否为1
    ...
    const int start_idx = atomicAdd(counter, N_samples);//counter[0]存已经处理过的采样点数
    const int ray_count = atomicAdd(counter+1, 1);//counter[2]存已经处理过的rays数，并返回old值

    rays_a[ray_count][0] = r;//已经处理的第ray_count条ray是编号为r的ray
...
__global__ void raymarching_test_kernel(
    ...
    while (t<t2 && s<N_samples){
        ...
        const float dt = calc_dt(t, exp_step_factor, max_samples, grid_size, cascades);//真实空间的绝对量
        const int mip = max(mip_from_pos(x, y, z, cascades),
                            mip_from_dt(dt, grid_size, cascades));
        ...
        const uint32_t idx = mip*grid_size3 + __morton3D(nx, ny, nz);//每一个mip对应的网格都需要存下来
        // 比如3.75/4 和 7/8，这两个点的morton码是一模一样的，因此用不同的mip来区分
        const bool occ = density_bitfield[idx/8] & (1<<(idx%8));
