...
def average_poses(poses, pts3d=None):
    ...
    center = torch.zeros([3,1])
    if pts3d is not None:
        center[2,0] = pts3d.mean(dim=0)[2]
        poses_center = poses[...,3].mean(0)
        center[0,0] = poses_center[0]
        center[1,0] = poses_center[1]
    else:
        center = poses[..., 3].mean(0)

    pose_avg = torch.cat([torch.eye(3), center], dim=1) # (3, 4)
...
def center_poses(poses, pts3d=None):
    ...
    pose_avg_inv = torch.linalg.inv(pose_avg_homo)
    last_row = torch.tile(torch.tensor([0, 0, 0, 1]), (len(poses), 1, 1)) # (N_images, 1, 4)
    poses_homo = \
        torch.cat([poses, last_row], 1) # (N_images, 4, 4) homogeneous coordinate
