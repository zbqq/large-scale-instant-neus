...
MAX_SAMPLES = 1024
NEAR_DISTANCE = 0.01
def render_without_nerfacc(model,ngp, rays_o, rays_d):# [N_samples 3]
# def render(self,sdf_net,color_net, rays_o, rays_d):# [N_samples 3]
    """_summary_
    Args:
    ...
    deltas, ts, total_samples) = RayMarcher.apply(#N_samples 1
        rays_o, rays_d, hits_t[:,0],model.density_bitfield,
            ...
            ngp.exp_step_factor,ngp.grid_size, MAX_SAMPLES)
    
    sigmas,rgbs = ngp(xyzs,dirs,retu_fea=True)
    # sdf,fea = sdf_net(xyzs,retu_fea=True)
